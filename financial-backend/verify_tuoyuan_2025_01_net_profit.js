...
async function verifyTuoyuan2025_01NetProfitCalculation() {
    ...
    try {
        ...
        console.log('🏢 公司层面成本中心费用：');
        console.log(`  📉 营销费用(市场部): ${totalMarketingCost.toFixed(2)} 万元`);
        console.log(`  📉 管理费用(企管部+总经理室): ${totalManagementCost.toFixed(2)} 万元`);
        console.log(`  📉 财务费用(财务部): ${totalFinanceCost.toFixed(2)} 万元`);
        console.log(`  💰 成本中心费用合计: ${(totalMarketingCost + totalManagementCost + totalFinanceCost).toFixed(2)} 万元\n`);

        // 拓源的客户列表
        const customers = ['电业项目', '用户项目', '用户设备', '贸易', '代理设备', '代理工程', '代理设计', '其他'];

        console.log('📊 各项目2025年1月净利润明细（不含成本中心费用）：\n');
        ...
        for (const customerName of customers) {
            ...
            // 3. 获取成本中心费用（营销、管理、财务）- 从成本中心结构表获取
            const [centerRows] = await connection.execute(
                'SELECT marketing, management, finance FROM tuoyuan_cost_center_structure WHERE period = ? AND customer_name = ?',
                [period, customerName]
            );
