...
router.get('/:period', async (req, res) => {
    try {
        ...
        console.log(`获取拓源在产情况数据 - 期间: ${period}`);
        ...
        console.error('获取拓源在产情况数据失败:', error);
        res.status(500).json({ 
            success: false, 
            message: '获取数据失败', 
            error: error.message 
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        console.log(`保存拓源在产情况数据 - 期间: ${period}, 项目数量: ${data.items.length}`);
        
        await connection.beginTransaction();
        
        // 删除指定期间的现有数据
        await connection.execute(
            'DELETE FROM tuoyuan_inventory_in_progress WHERE period = ?',
            [period]
        );
        ...
        console.error('保存拓源在产情况数据失败:', error);
        res.status(500).json({ 
            success: false, 
            message: '保存数据失败', 
            error: error.message 
