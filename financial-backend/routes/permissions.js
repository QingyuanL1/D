  ...
  try {
    ...
    const [permissionRows] = await pool.execute(`
      SELECT fm.id, fm.module_name, fm.module_key, fm.module_category,
             fm.route_path, fm.description, fm.display_order, rp.permission_type
      FROM role_permissions rp
      JOIN form_modules fm ON rp.module_id = fm.id
      JOIN users u ON u.role_id = rp.role_id
      WHERE u.id = ?
      ORDER BY fm.module_category,
               CASE WHEN fm.display_order > 0 THEN fm.display_order ELSE 999 END,
               fm.module_name
    `, [userId]);
...
// 获取用户权限信息（按公司过滤）
router.get('/user/:userId/company/:companyName', async (req, res) => {
  try {
    ...
    // 定义允许的分类（该公司的模块 + 通用模块）
    const allowedCategories = [companyName, '财务', '营销', '生产', '企管'];
    ...
    const [permissionRows] = await pool.execute(`
      ...
      JOIN form_modules fm ON rp.module_id = fm.id
      JOIN users u ON u.role_id = rp.role_id
      WHERE u.id = ? AND fm.module_category IN (${allowedCategories.map(() => '?').join(',')})
      ORDER BY fm.module_category,
               CASE WHEN fm.display_order > 0 THEN fm.display_order ELSE 999 END,
               fm.module_name
    ...
    const [pendingForms] = await pool.execute(`
      ...
      LEFT JOIN form_submissions fs ON fm.id = fs.module_id AND fs.period = ?
      WHERE u.id = ? AND rp.permission_type = 'write' AND fs.id IS NULL 
        AND fm.module_category IN (${allowedCategories.map(() => '?').join(',')})
      ORDER BY fm.module_category,
               CASE WHEN fm.display_order > 0 THEN fm.display_order ELSE 999 END,
               fm.module_name
    `, [currentPeriod, userId, ...allowedCategories]);
...
router.get('/modules', async (req, res) => {
  try {
    const [rows] = await pool.execute(`
      SELECT id, module_name, module_key, module_category, route_path, description, display_order
      FROM form_modules
  ...
  try {
    ...
    const [permissionRows] = await pool.execute(`
      SELECT rp.id, rp.module_id, rp.permission_type,
             fm.module_name, fm.module_key, fm.module_category, fm.display_order
      FROM role_permissions rp
      JOIN form_modules fm ON rp.module_id = fm.id
      WHERE rp.role_id = ?
      ORDER BY fm.module_category,
