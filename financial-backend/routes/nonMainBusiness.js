...
router.get('/:period', createBudgetMiddleware('non_main_business'), async (req, res) => {
    ...
    try {
        ...
        if (currentData.length > 0) {
            // 有当前期间数据时，合并当前期间和累计数据
            finalData = currentData.map(item => {
                const key = `${item.category}-${item.id}`;
        ...
        } else if (Object.keys(cumulativeData).length > 0) {
            // 没有当前期间数据但有累计数据时，使用累计数据生成结果
            finalData = Object.values(cumulativeData).map(item => ({
                id: item.id,
                category: item.category,
                yearlyPlan: item.yearlyPlan,
                currentPeriod: 0, // 当前期间为0
                cumulative: item.cumulative
            }));
        } else {
            // 既没有当前期间数据也没有累计数据，返回默认结构
            const defaultData = [
