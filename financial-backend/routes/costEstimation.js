...
router.get('/:period', createBudgetMiddleware('cost_estimate_personnel_withdrawals'), async (req, res) => {
    try {
        ...
        const query = `
            SELECT category, customer_type, initial_balance, current_period, current_write_off, year_total, provision_rate
        ...
        const data = {
            equipment: rows.filter(row => row.category === '设备').map(row => ({
                ...
                currentWriteOff: row.current_write_off,
...
router.post('/', async (req, res) => {
    try {
        ...
        if (insertData.length > 0) {
            ...
            const insertQuery = `
                INSERT INTO cost_estimation (period, category, customer_type, initial_balance, current_period, current_write_off, year_total, provision_rate)
                VALUES ${placeholders}
            `;
