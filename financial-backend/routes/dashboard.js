...
router.get('/user/:userId', async (req, res) => {
  try {
    ...
    const currentPeriod = currentDate.toISOString().slice(0, 7); // YYYY-MM 上个月
...
// 获取公司特定仪表板数据
router.get('/company/:userId/:companyName', async (req, res) => {
  try {
    ...
    const [permissionStats] = await pool.execute(`
      ...
      WHERE u.id = ? AND fm.module_category = ?
    `, [userId, companyName]);
    ...
    const [currentMonthStats] = await pool.execute(`
      ...
      JOIN users u ON u.role_id = rp.role_id
      LEFT JOIN form_submissions fs ON fm.id = fs.module_id AND fs.period = ?
      WHERE u.id = ? AND rp.permission_type = 'write' AND fm.module_category = ?
    `, [currentPeriod, userId, companyName]);
    ...
    const [pendingForms] = await pool.execute(`
      ...
      JOIN users u ON u.role_id = rp.role_id
      LEFT JOIN form_submissions fs ON fm.id = fs.module_id AND fs.period = ?
      WHERE u.id = ? AND rp.permission_type = 'write' AND fm.module_category = ? AND fs.id IS NULL
      ORDER BY fm.module_name
      LIMIT 10
    `, [currentPeriod, userId, companyName]);

    // 获取该公司按分类的提交统计（实际上只会有一个分类）
    const [categoryStats] = await pool.execute(`
      SELECT 
        fm.module_category,
        COUNT(CASE WHEN rp.permission_type = 'write' THEN 1 END) as total_writable,
    ...
    console.error('获取公司仪表板数据失败:', error);
    res.status(500).json({ error: '获取公司仪表板数据失败' });
  }
});
