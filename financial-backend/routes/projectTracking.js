...
router.get('/:period', attachProjectTrackingBudget, async (req, res) => {
    try {
        ...
        const query = `
            SELECT category, customer, yearly_plan, current_period, current_total, progress 
...
router.post('/', async (req, res) => {
    try {
        ...
        if (insertData.length > 0) {
            ...
            const insertQuery = `
                INSERT INTO project_tracking (period, category, customer, yearly_plan, current_period, current_total, progress) 
                VALUES ${placeholders}
            `;
