  ...
  try {
    ...
    for (const [fieldName, fieldData] of Object.entries(parsedData)) {
      ...
      if (!fieldData.hasOwnProperty('current_amount') || !fieldData.hasOwnProperty('cumulative_amount')) {
        return { valid: false, error: `字段 ${fieldName} 缺少必需的属性：current_amount 或 cumulative_amount` };
      }
      ...
      if (fieldData.cumulative_amount !== null && typeof fieldData.cumulative_amount !== 'number') {
        return { valid: false, error: `字段 ${fieldName} 的 cumulative_amount 必须是数字或null` };
      }
    }
...
async function calculateCumulativeAmounts(targetPeriod, currentData) {
  try {
    ...
    const month = targetDate.month() + 1; // moment.js月份从0开始
    ...
    const [historicalRows] = await pool.query(
      'SELECT period, data FROM changzhou_tuoyuan_income_statement WHERE period >= ? AND period < ? ORDER BY period ASC',
...
router.post('/', async (req, res) => {
  try {
    ...
    console.log('保存常州拓源利润表请求 - 期间:', period, '数据类型:', typeof data, '自动计算累计:', autoCalculateCumulative);
    ...
    const [existing] = await pool.query(
      'SELECT id FROM changzhou_tuoyuan_income_statement WHERE period = ?',
      [formattedPeriod]
    );
    ...
    if (existing.length > 0) {
      // 更新现有数据
      await pool.query(
        'UPDATE changzhou_tuoyuan_income_statement SET data = ?, updated_at = NOW() WHERE period = ?',
        [dataJson, formattedPeriod]
      );
      console.log('常州拓源利润表数据更新成功，期间:', formattedPeriod);
      res.json({ 
        success: true, 
        message: '常州拓源利润表数据更新成功', 
        action: 'updated',
      ...
      console.log('常州拓源利润表数据保存成功，期间:', formattedPeriod);
      res.json({ 
        success: true, 
        message: '常州拓源利润表数据保存成功', 
        action: 'created',
        calculatedData: autoCalculateCumulative ? finalData : undefined
      });
    }
  } catch (error) {
    console.error('保存常州拓源利润表数据时出错:', error);
    res.status(500).json({ error: '服务器内部错误' });
  }
});

// 获取指定年份的常州拓源利润表数据（用于图表显示）
router.get('/annual/:year', async (req, res) => {
  try {
    ...
    console.error('获取全年常州拓源利润表数据时出错:', error);
    res.status(500).json({ error: '服务器内部错误' });
  }
});
