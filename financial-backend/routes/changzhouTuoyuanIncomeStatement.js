...
router.post('/', async (req, res) => {
  try {
    ...
    console.log('保存常州拓源利润表请求 - 期间:', period, '数据类型:', typeof data);
    ...
    if (existing.length > 0) {
      ...
      res.json({ success: true, message: '常州拓源利润表数据更新成功', action: 'updated' });
    } else {
      // 插入新数据
      await pool.query(
      ...
      res.json({ success: true, message: '常州拓源利润表数据保存成功', action: 'created' });
    }
  } catch (error) {
