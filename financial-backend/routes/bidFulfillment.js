...
// 获取中标未履约情况数据
router.get('/:period', createBudgetMiddleware('bid_fulfillment_status'), async (req, res) => {
    try {
        ...
        console.error('获取中标未履约情况数据失败:', error);
        res.status(500).json({ 
            success: false, 
            message: '服务器内部错误' 
        });
...
router.post('/', async (req, res) => {
    try {
        ...
        // 删除该期间的现有数据
        await pool.execute('DELETE FROM bid_fulfillment WHERE period = ?', [period]);
        ...
        if (insertData.length > 0) {
            ...
            const insertQuery = `
                INSERT INTO bid_fulfillment (period, category, customer_type, initial_amount, current_amount) 
                VALUES ${placeholders}
            `;
        ...
        res.json({
            success: true,
            message: '中标未履约情况数据保存成功',
            period: period
        });
    } catch (error) {
        console.error('保存中标未履约情况数据失败:', error);
        res.status(500).json({ 
            success: false, 
            message: '服务器内部错误' 
        });
...
router.put('/:period', async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: '中标未履约情况数据更新成功',
            period: period
        });
    } catch (error) {
        console.error('更新中标未履约情况数据失败:', error);
        res.status(500).json({ 
            success: false, 
            message: '服务器内部错误' 
        });
...
router.delete('/:period', async (req, res) => {
    try {
        ...
        const deleteQuery = 'DELETE FROM bid_fulfillment WHERE period = ?';
        ...
        res.json({
            success: true,
            message: '中标未履约情况数据删除成功',
            period: period
        });
    } catch (error) {
        console.error('删除中标未履约情况数据失败:', error);
        res.status(500).json({ 
            success: false, 
            message: '服务器内部错误' 
        });
