...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://127.0.0.1:3000/nanhua-new-orders/${targetPeriod}`)
        if (!response.ok) {
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://127.0.0.1:3000/forms/submission/${MODULE_IDS.NANHUA_NEW_ORDERS}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://127.0.0.1:3000/nanhua-new-orders', {
            method: 'POST',
            headers: {
