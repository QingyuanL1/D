...
const fixedPlanData: StockData = {
    customers: [
        ...
        { customerName: '其他', stockBalance: 0.00, current: 0, accumulated: 0, conversionRate: 0 }
    ]
}
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/nanhua-stock-order-to-income/${targetPeriod}`)
        if (!response.ok) {
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/forms/submission/${MODULE_IDS.NANHUA_STOCK_ORDER_TO_INCOME}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://47.111.95.19:3000/nanhua-stock-order-to-income', {
            method: 'POST',
            headers: {
