<template>
    <div class="max-w-[1500px] mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">主营业务——当年订单转收入</h1>
            ...
            <table class="w-full border-collapse border border-gray-300">
                <thead class="sticky top-0 bg-white">
                    <tr class="bg-gray-50">
                        ...
                        <th class="border border-gray-300 px-4 py-2">当年新签合同累计</th>
                        <th class="border border-gray-300 px-4 py-2">当期</th>
                        <th class="border border-gray-300 px-4 py-2">累计</th>
                        <th class="border border-gray-300 px-4 py-2">当年订单转收入完成率</th>
                    </tr>
                </thead>
                <tbody>
                    <!-- 客户列表 -->
                    <template v-for="(item, index) in orderData.customers" :key="`customer-${index}`">
                        <tr>
                            <td v-if="index === 0" :rowspan="orderData.customers.length" class="border border-gray-300 px-4 py-2 font-medium text-center">
                                工程
                            </td>
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-right">
                                <span class="text-sm font-medium">{{ calculateCompletionRate(item.newContractTotal, item.accumulated) }}%</span>
                            </td>
                        </tr>
                    </template>
                    ...
                    <tr class="bg-gray-50 font-bold">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-right">
                            {{ formatNumber(totalData.newContractTotal) }}
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-right">
        ...
        <FormAttachmentAndRemarks 
            :module-id="MODULE_IDS.NANHUA_ORDER_TO_INCOME"
            :period="period"
...
// 固定的年度计划数据
const fixedPlanData: OrderData = {
    customers: [
        { customerName: '一包项目', newContractTotal: 2678.00, current: 0, accumulated: 0, completionRate: 0 },
        { customerName: '二包项目', newContractTotal: 110.00, current: 0, accumulated: 0, completionRate: 0 },
        { customerName: '域内合作项目', newContractTotal: 200.00, current: 0, accumulated: 0, completionRate: 0 },
...
const orderData = ref<OrderData>(JSON.parse(JSON.stringify(fixedPlanData)))
...
// 计算完成率函数
const calculateCompletionRate = (newContractTotal: number, accumulated: number): string => {
    if (!newContractTotal || newContractTotal === 0) return '0.00'
    const rate = (accumulated / newContractTotal) * 100
...
const totalData = computed(() => {
    ...
    // 计算总完成率
    total.completionRate = total.newContractTotal > 0 ? (total.accumulated / total.newContractTotal * 100) : 0
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/nanhua-order-to-income/${targetPeriod}`)
        if (!response.ok) {
        ...
        if (result.data && result.data.customers) {
            // 直接使用后端返回的数据
            orderData.value.customers = result.data.customers.map((item: any) => ({
                customerName: item.customerName,
                newContractTotal: Number(item.newContractTotal) || 0,
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/forms/submission/${MODULE_IDS.NANHUA_ORDER_TO_INCOME}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://47.111.95.19:3000/nanhua-order-to-income', {
            method: 'POST',
            headers: {
        ...
        // 记录提交状态（包含备注和建议）
        await recordFormSubmission(MODULE_IDS.NANHUA_ORDER_TO_INCOME, period.value, orderData.value, remarks.value, suggestions.value)
...
const handleReset = () => {
    orderData.value = JSON.parse(JSON.stringify(fixedPlanData))
    remarks.value = ''
    suggestions.value = ''
}
