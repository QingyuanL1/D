        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div class="bg-white p-6 rounded-lg border border-gray-200 hover:border-blue-400 hover:shadow-sm transition-all relative">
            ...
            <div class="mb-4">
              <div style="width: 100%; height: 8px; background-color: #e5e7eb; border-radius: 4px;">
                <div 
                  style="height: 8px; border-radius: 4px; background-color: #16a34a; transition: width 0.3s ease;"
            ...
            <router-link to="/analytics/tuoyuan-profit-margin-chart" class="block w-full text-center py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors" @click="handleNavigation">
              质量情况
            </router-link>
          </div>
...
const fetchDashboardData = async () => {
  try {
    ...
    const response = await fetch(`http://127.0.0.1:3000/dashboard/company/${userId}/${encodeURIComponent(companyName)}`)
...
const fetchTuoyuanProfitMarginData = async () => {
  try {
    const currentYear = new Date().getFullYear()
    const response = await fetch(`http://127.0.0.1:3000/analytics/tuoyuan-profit-margin/${currentYear}`)
