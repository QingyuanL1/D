<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
      ...
      <!-- 数据分析入口 -->
      <div class="bg-white p-6 rounded-lg shadow-sm mb-8">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">拓源公司数据分析中心</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div
            ...
            <router-link to="/analytics/net-profit-chart"
          ...
          <div
            ...
            <div class="mb-4">
              <div style="width: 100%; height: 8px; background-color: #e5e7eb; border-radius: 4px;">
                <div style="height: 8px; border-radius: 4px; background-color: #2563eb; transition: width 0.3s ease;"
                  :style="`width: ${roeData.roe > 0 ? Math.min(roeData.roe, 100) : 0}%;`"></div>
              </div>
            </div>
            <router-link to="/analytics/roe-chart"
              class="block w-full text-center py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
              @click="handleNavigation">
              质量情况
            </router-link>
          ...
          <div
            ...
            <div class="flex items-center justify-between mb-4">
              <div class="p-2 bg-blue-100 rounded-md">
                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M9 7h6m0 10v-3m-3 3h3m0 0v-3m0 3h3m-6 0h6m-6-3v3"></path>
                </svg>
              </div>
              <div class="text-right">
          ...
          <div
            ...
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="text-right">
                <div class="text-sm font-semibold text-green-600">{{ tuoyuanProfitMarginData.currentRate }}%</div>
                <div class="text-xs text-gray-500">毛利率</div>
              </div>
            </div>
            <h4 class="text-lg font-medium text-gray-900 mb-2">毛利率分析</h4>
            <p class="text-sm text-gray-600 mb-3 h-12">分析拓源公司毛利率趋势与板块表现</p>
            <div class="mb-4">
              <div style="width: 100%; height: 8px; background-color: #e5e7eb; border-radius: 4px;">
                <div style="height: 8px; border-radius: 4px; background-color: #16a34a; transition: width 0.3s ease;"
                  :style="`width: ${tuoyuanProfitMarginData.currentRate > 0 ? Math.min(tuoyuanProfitMarginData.currentRate / tuoyuanProfitMarginData.targetRate * 100, 100) : 3}%;`">
                </div>
              </div>
            </div>
            <router-link to="/analytics/tuoyuan-profit-margin-chart"
              class="block w-full text-center py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors"
              @click="handleNavigation">
              质量情况
            </router-link>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-6">
          <!-- 营业结构分析卡片 -->
          <div
            class="bg-white p-6 rounded-lg border border-gray-200 hover:border-purple-400 hover:shadow-sm transition-all relative">
            <div class="flex items-center justify-between mb-4">
            ...
            <div class="mb-4">
              <div style="width: 100%; height: 8px; background-color: #e5e7eb; border-radius: 4px;">
                <div style="height: 8px; border-radius: 4px; background-color: #9333ea; transition: width 0.3s ease;"
                  :style="`width: ${Math.max(businessStructureData.equipmentPercentage, 3)}%;`"></div>
              </div>
            </div>
            <router-link to="/analytics/tuoyuan-business-structure"
              class="block w-full text-center py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition-colors"
              @click="handleNavigation">
              查看分析
            </router-link>
          </div>
          ...
          <!-- 边际贡献率趋势分析卡片 -->
          <div
            class="bg-white p-6 rounded-lg border border-gray-200 hover:border-orange-400 hover:shadow-sm transition-all relative">
            <div class="flex items-center justify-between mb-4">
            ...
            <div class="mb-4">
              <div style="width: 100%; height: 8px; background-color: #e5e7eb; border-radius: 4px;">
                <div style="height: 8px; border-radius: 4px; background-color: #ea580c; transition: width 0.3s ease;"
            ...
            <router-link to="/analytics/tuoyuan-contribution-rate-chart"
              class="block w-full text-center py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 transition-colors"
              @click="handleNavigation">
              趋势分析
            </router-link>
          </div>
          ...
          <!-- 新签订单趋势分析卡片 -->
          <div
            class="bg-white p-6 rounded-lg border border-gray-200 hover:border-indigo-400 hover:shadow-sm transition-all relative">
            <div class="flex items-center justify-between mb-4">
            ...
            <div class="mb-4">
              <div style="width: 100%; height: 8px; background-color: #e5e7eb; border-radius: 4px;">
                <div style="height: 8px; border-radius: 4px; background-color: #6366f1; transition: width 0.3s ease;"
                  :style="`width: ${Math.max(newOrdersProgress, 3)}%;`"></div>
              </div>
            </div>
            <router-link to="/analytics/tuoyuan-new-order-chart"
              class="block w-full text-center py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors"
              @click="handleNavigation">
              查看趋势
            </router-link>
          </div>
...
// 拓源毛利率数据
const tuoyuanProfitMarginData = ref({
  currentRate: 0,
  targetRate: 24.99
})
...
const fetchDashboardData = async () => {
  try {
    ...
    const companyName = companyMapping[selectedCompany] || '拓源公司'  // 默认为拓源公司
...
const fetchTuoyuanProfitMarginData = async () => {
  try {
    const currentYear = new Date().getFullYear()
    const response = await fetch(`http://47.111.95.19:3000/analytics/tuoyuan-profit-margin/${currentYear}`)

    if (!response.ok) {
      throw new Error('获取拓源毛利率数据失败')
    }

    const result = await response.json()
    if (result.success && result.data) {
      tuoyuanProfitMarginData.value.currentRate = result.data.currentRate || 0
      tuoyuanProfitMarginData.value.targetRate = result.data.targetRate || 24.99
    } else {
      console.warn('拓源毛利率数据获取失败:', result.message)
