<template>
    <div class="max-w-[1200px] mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">常州拓源电气有限公司 - 利润表(主表)（单位：万元）</h1>
...
const loadData = async (targetPeriod: string) => {
  try {
    ...
    const response = await fetch(`http://127.0.0.1:3000/financial-reports/tuoyuan/income-statement/${targetPeriod}`, {
      method: 'GET',
      headers: {
...
const handleSave = async () => {
  try {
    ...
    const response = await fetch('http://127.0.0.1:3000/financial-reports/tuoyuan/income-statement', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
