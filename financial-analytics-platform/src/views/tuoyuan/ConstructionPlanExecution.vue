<template>
    <div class="max-w-[1500px] mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">施工计划执行情况</h1>
            ...
            <table class="w-full border-collapse border border-gray-300">
                <thead class="sticky top-0 bg-white">
                    <tr class="bg-gray-50">
                        ...
                        <th class="border border-gray-300 px-4 py-2 w-24">年度目标</th>
                        <th class="border border-gray-300 px-4 py-2 w-24">准时完工率</th>
                        <th class="border border-gray-300 px-4 py-2 w-24">验收合格率</th>
                        <th class="border border-gray-300 px-4 py-2 w-24">计划达成率</th>
                    </tr>
                </thead>
                <tbody>
                    <template v-for="(item, index) in constructionData.items" :key="`construction-${index}`">
                        <tr>
                            ...
                            <td class="border border-gray-300 px-4 py-2">
                                <input 
                                    v-model="item.onTimeCompletionRate" 
                                    type="number" 
                                    class="w-full px-2 py-1 border rounded text-right" 
                                    step="0.01"
                                    min="0"
                            ...
                            <td class="border border-gray-300 px-4 py-2">
                                <input 
                                    v-model="item.acceptanceQualificationRate" 
                                    type="number" 
                                    class="w-full px-2 py-1 border rounded text-right" 
                                    step="0.01"
                                    min="0"
                            ...
                            <td class="border border-gray-300 px-4 py-2">
                                <input 
                                    v-model="item.planAchievementRate" 
                                    type="number" 
                                    class="w-full px-2 py-1 border rounded text-right" 
                                    step="0.01"
                                    min="0"
                    ...
                    <tr class="bg-gray-50 font-bold">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-right">
                            {{ formatNumber(totalData.yearlyTarget) }}
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-right">
                            <span class="text-sm font-bold">{{ formatPercentage(totalData.onTimeCompletionRate) }}%</span>
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-right">
                            <span class="text-sm font-bold">{{ formatPercentage(totalData.acceptanceQualificationRate) }}%</span>
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-right">
                            <span class="text-sm font-bold">{{ formatPercentage(totalData.planAchievementRate) }}%</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <FormAttachmentAndRemarks 
            :module-id="MODULE_IDS.TUOYUAN_CONSTRUCTION_PLAN_EXECUTION"
            :period="period"
...
const isFirstInSegment = (index: number): boolean => {
    ...
    return constructionData.value.items[index].segmentAttribute !== constructionData.value.items[index - 1].segmentAttribute
}

// 计算板块的行数
const getSegmentRowspan = (segmentAttribute: string): number => {
    return constructionData.value.items.filter(item => item.segmentAttribute === segmentAttribute).length
}
...
const totalData = computed(() => {
    ...
    const validItems = constructionData.value.items.filter(item => item.yearlyTarget > 0 || item.onTimeCompletionRate > 0 || item.acceptanceQualificationRate > 0 || item.planAchievementRate > 0)
    
    constructionData.value.items.forEach(item => {
        total.yearlyTarget += item.yearlyTarget || 0
    ...
    // 计算平均值
    const itemCount = constructionData.value.items.length
    if (itemCount > 0) {
        total.onTimeCompletionRate = total.onTimeCompletionRate / itemCount
        total.acceptanceQualificationRate = total.acceptanceQualificationRate / itemCount
        total.planAchievementRate = total.planAchievementRate / itemCount
    }
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/tuoyuan-construction-plan-execution/${targetPeriod}`)
        if (!response.ok) {
        ...
        if (result.data && result.data.items) {
            constructionData.value.items = result.data.items.map((item: any) => ({
                ...
                yearlyTarget: Number(item.yearlyTarget) || 0,
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/forms/submission/${MODULE_IDS.TUOYUAN_CONSTRUCTION_PLAN_EXECUTION}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://47.111.95.19:3000/tuoyuan-construction-plan-execution', {
            method: 'POST',
            headers: {
        ...
        await recordFormSubmission(MODULE_IDS.TUOYUAN_CONSTRUCTION_PLAN_EXECUTION, period.value, { items: constructionData.value.items }, remarks.value, suggestions.value)
        alert('保存成功')
    } catch (error) {
