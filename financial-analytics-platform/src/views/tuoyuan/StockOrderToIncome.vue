...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/forms/submission/${MODULE_IDS.STOCK_ORDER_TO_INCOME}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://47.111.95.19:3000/tuoyuan-stock-order-to-income', {
            method: 'POST',
            headers: {
