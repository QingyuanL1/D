    ...
    try {
        ...
        for (let item of orderData.value.items) {
            ...
            for (let m = 1; m < currentMonth; m++) {
                ...
                try {
                    const response = await fetch(`http://127.0.0.1:3000/tuoyuan-new-order-structure/${monthPeriod}`)
                    if (response.ok) {
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://127.0.0.1:3000/tuoyuan-new-order-structure/${targetPeriod}`)
        if (!response.ok) {
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://127.0.0.1:3000/forms/submission/${MODULE_IDS.TUOYUAN_NEW_ORDER_STRUCTURE}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://127.0.0.1:3000/tuoyuan-new-order-structure', {
            method: 'POST',
            headers: {
