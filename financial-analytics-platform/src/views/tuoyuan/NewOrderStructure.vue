<template>
    <div class="max-w-[1500px] mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">拓源新签订单结构与质量</h1>
        ...
        <div class="overflow-x-auto my-6">
            <table class="w-full border-collapse border border-gray-300">
                <thead class="sticky top-0 bg-white">
                    <tr class="bg-gray-50">
                        <th class="border border-gray-300 px-4 py-2 w-32">板块属性</th>
                ...
                <tbody>
                    <template v-for="(item, index) in orderData.items" :key="`item-${index}`">
                        <tr>
        ...
        <FormAttachmentAndRemarks 
            :module-id="MODULE_IDS.TUOYUAN_NEW_ORDER_STRUCTURE"
            :period="period"
...
const fixedPlanData: OrderData = {
    items: [
        { segmentAttribute: '设备', customerAttribute: '电业项目', annualPlan: 7800.00, currentPeriod: 0, currentCumulative: 0, executionProgress: 0 },
        { segmentAttribute: '设备', customerAttribute: '用户项目', annualPlan: 0.00, currentPeriod: 0, currentCumulative: 0, executionProgress: 0 },
        { segmentAttribute: '设备', customerAttribute: '贸易', annualPlan: 800.00, currentPeriod: 0, currentCumulative: 0, executionProgress: 0 },
        { segmentAttribute: '设备', customerAttribute: '代理设备', annualPlan: 3000.00, currentPeriod: 0, currentCumulative: 0, executionProgress: 0 },
...
const isFirstInSegment = (index: number): boolean => {
    ...
    return orderData.value.items[index].segmentAttribute !== orderData.value.items[index - 1].segmentAttribute
}

// 计算板块的行数
const getSegmentRowspan = (segmentAttribute: string): number => {
    return orderData.value.items.filter(item => item.segmentAttribute === segmentAttribute).length
}
...
const calculateCurrentCumulative = async (targetPeriod: string) => {
    try {
        ...
        // 为每个项目计算当期累计
        for (let item of orderData.value.items) {
            ...
            for (let m = 1; m < currentMonth; m++) {
                ...
                try {
                    const response = await fetch(`http://47.111.95.19:3000/tuoyuan-new-order-structure/${monthPeriod}`)
                    if (response.ok) {
...
watch(() => orderData.value.items, async (newItems) => {
    // 当当期发生变化时，重新计算当期累计
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/tuoyuan-new-order-structure/${targetPeriod}`)
        if (!response.ok) {
        ...
        if (result.data && result.data.items) {
            orderData.value.items = result.data.items.map((item: any) => ({
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/forms/submission/${MODULE_IDS.TUOYUAN_NEW_ORDER_STRUCTURE}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://47.111.95.19:3000/tuoyuan-new-order-structure', {
            method: 'POST',
            headers: {
        ...
        await recordFormSubmission(MODULE_IDS.TUOYUAN_NEW_ORDER_STRUCTURE, period.value, { items: orderData.value.items }, remarks.value, suggestions.value)
        alert('保存成功')
    } catch (error) {
