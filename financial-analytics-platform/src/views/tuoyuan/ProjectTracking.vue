<template>
    <div class="max-w-[1500px] mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">拓源项目跟踪情况</h1>
...
const fixedPlanData: TrackingData = {
    items: [
        { segmentAttribute: '设备', customerAttribute: '电业项目', annualPlan: 0.00, currentPeriod: 0, currentCumulative: 0, executionProgress: 0 },
...
const trackingData = ref<TrackingData>(JSON.parse(JSON.stringify(fixedPlanData)))
const remarks = ref('')
...
const isFirstInSegment = (index: number): boolean => {
    ...
    return trackingData.value.items[index].segmentAttribute !== trackingData.value.items[index - 1].segmentAttribute
}

// 计算板块的行数
const getSegmentRowspan = (segmentAttribute: string): number => {
    return trackingData.value.items.filter(item => item.segmentAttribute === segmentAttribute).length
}
...
watch(() => trackingData.value.items, async (newItems) => {
    // 当当期发生变化时，重新计算当期累计
...
const loadData = async (targetPeriod: string) => {
    try {
        ...
        if (result.data && result.data.items) {
            trackingData.value.items = result.data.items.map((item: any) => ({
...
const resetToDefaultData = () => {
    trackingData.value = JSON.parse(JSON.stringify(fixedPlanData))
}
    ...
    try {
        const response = await fetch('http://127.0.0.1:3000/tuoyuan-project-tracking', {
            ...
            body: JSON.stringify({
                period: period.value,
                data: {
                    items: trackingData.value.items
                }
            })
        })
        ...
        await recordFormSubmission(MODULE_IDS.TUOYUAN_PROJECT_TRACKING, period.value, { items: trackingData.value.items }, remarks.value, suggestions.value)
        alert('保存成功')
    } catch (error) {
