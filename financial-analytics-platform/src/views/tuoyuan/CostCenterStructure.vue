<template>
  <div class="max-w-[1600px] mx-auto bg-white rounded-lg shadow-lg p-6">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold">拓源成本中心结构与质量（按年度计划口径分解）（单位：万元）</h1>
      ...
      <table class="w-full border-collapse border border-gray-300">
        ...
        <tbody>
          ...
          <tr v-for="(item, index) in equipmentData" :key="`equipment-${index}`"
            ...
            <td class="border border-gray-300 px-4 py-2" :class="item.isCategory ? 'font-medium bg-blue-50' : ''">
              {{ item.customerType }}
            </td>
            ...
            <td class="border border-gray-300 px-4 py-2">
              <input v-if="!item.isCategory" v-model.number="item.currentMonthIncome" type="number"
                class="w-full px-2 py-1 border rounded text-right" step="0.01" @input="calculatePercentage(item)" />
              <span v-else class="font-medium">{{ formatNumber(item.currentMonthIncome) }}</span>
            </td>
            <!-- 营销 -->
            <td class="border border-gray-300 px-2 py-2">
              <input v-if="!item.isCategory" v-model.number="item.marketing" type="number"
                class="w-full px-1 py-1 border rounded text-right text-sm" step="0.01"
                @input="updateCurrentMonthFromBreakdown(item)" />
              <span v-else class="font-medium text-sm">{{ formatNumber(item.marketing) }}</span>
            </td>
            <!-- 管理 -->
            <td class="border border-gray-300 px-2 py-2">
              <input v-if="!item.isCategory" v-model.number="item.management" type="number"
                class="w-full px-1 py-1 border rounded text-right text-sm" step="0.01"
                @input="updateCurrentMonthFromBreakdown(item)" />
            ...
            <!-- 财务 -->
            <td class="border border-gray-300 px-2 py-2">
              <input v-if="!item.isCategory" v-model.number="item.finance" type="number"
                class="w-full px-1 py-1 border rounded text-right text-sm" step="0.01"
                @input="updateCurrentMonthFromBreakdown(item)" />
            ...
            <!-- 生产制造 -->
            <td class="border border-gray-300 px-2 py-2">
              <input v-if="!item.isCategory" v-model.number="item.production" type="number"
                class="w-full px-1 py-1 border rounded text-right text-sm" step="0.01"
                @input="updateCurrentMonthFromBreakdown(item)" />
          ...
          <!-- 其他板块 -->
          <tr v-for="(item, index) in otherData" :key="`other-${index}`" :class="item.isCategory ? 'bg-green-50' : ''">
            <td v-if="index === 0" class="border border-gray-300 px-4 py-2 font-medium bg-green-100 text-center"
              :rowspan="otherData.length">其他</td>
            <td class="border border-gray-300 px-4 py-2" :class="item.isCategory ? 'font-medium bg-green-50' : ''">
              {{ item.customerType }}
            </td>
            <td class="border border-gray-300 px-4 py-2">
              <span class="font-medium">{{ formatNumber(item.cumulativeIncome) }}</span>
          ...
          <tr class="bg-gray-100 font-bold">
            ...
            <td class="border border-gray-300 px-4 py-2 text-right">{{ formatNumber(totalData.currentMonthIncome) }}
            </td>
            <td class="border border-gray-300 px-2 py-2 text-right text-sm">{{ formatNumber(totalData.marketing) }}</td>
            <td class="border border-gray-300 px-2 py-2 text-right text-sm">{{ formatNumber(totalData.management) }}
            </td>
            <td class="border border-gray-300 px-2 py-2 text-right text-sm">{{ formatNumber(totalData.finance) }}</td>
            <td class="border border-gray-300 px-2 py-2 text-right text-sm">{{ formatNumber(totalData.production) }}
            </td>
...
const moduleId = MODULE_IDS.TUOYUAN_COST_CENTER_STRUCTURE
const remarks = ref('')
...
const equipmentData = ref<CostItem[]>([
  { customerType: '电业项目', cumulativeIncome: 0, currentMonthIncome: 0, marketing: 0, management: 0, finance: 0, production: 0, percentage: '0.00' },
  { customerType: '用户设备', cumulativeIncome: 0, currentMonthIncome: 0, marketing: 0, management: 0, finance: 0, production: 0, percentage: '0.00' },
  { customerType: '贸易', cumulativeIncome: 0, currentMonthIncome: 0, marketing: 0, management: 0, finance: 0, production: 0, percentage: '0.00' },
  { customerType: '代理设备', cumulativeIncome: 0, currentMonthIncome: 0, marketing: 0, management: 0, finance: 0, production: 0, percentage: '0.00' }
])

// 其他板块数据 - 根据您的图片要求
const otherData = ref<CostItem[]>([
  { customerType: '代理工程', cumulativeIncome: 0, currentMonthIncome: 0, marketing: 0, management: 0, finance: 0, production: 0, percentage: '0.00' },
  { customerType: '代理设计', cumulativeIncome: 0, currentMonthIncome: 0, marketing: 0, management: 0, finance: 0, production: 0, percentage: '0.00' }
])

// 合计数据
const totalData = computed(() => {
  const totalCumulative = equipmentData.value.reduce((sum, item) => sum + item.cumulativeIncome, 0) +
    otherData.value.reduce((sum, item) => sum + item.cumulativeIncome, 0)

  const totalCurrent = equipmentData.value.reduce((sum, item) => sum + item.currentMonthIncome, 0) +
    otherData.value.reduce((sum, item) => sum + item.currentMonthIncome, 0)

  const totalMarketing = equipmentData.value.reduce((sum, item) => sum + (item.marketing || 0), 0) +
    otherData.value.reduce((sum, item) => sum + (item.marketing || 0), 0)

  const totalManagement = equipmentData.value.reduce((sum, item) => sum + (item.management || 0), 0) +
    otherData.value.reduce((sum, item) => sum + (item.management || 0), 0)

  const totalFinance = equipmentData.value.reduce((sum, item) => sum + (item.finance || 0), 0) +
    otherData.value.reduce((sum, item) => sum + (item.finance || 0), 0)

  const totalProduction = equipmentData.value.reduce((sum, item) => sum + (item.production || 0), 0) +
    otherData.value.reduce((sum, item) => sum + (item.production || 0), 0)
...
const getCategoryTypeByItem = (item: CostItem) => {
  ...
  if (otherData.value.includes(item)) return 'otherData'
  return null
}
...
const loadAllMonthsData = async (currentPeriod: string) => {
  try {
    ...
    for (let i = 1; i <= currentMonth; i++) {
      ...
      try {
        const response = await fetch(`http://47.111.95.19:3000/tuoyuan-cost-center-structure/${monthPeriod}`)
        if (response.ok) {
...
const calculateCumulativeIncome = (categoryType: string, customerType: string) => {
  ...
  else if (categoryType === 'otherData') currentData = otherData.value
...
const updateCumulativeIncome = () => {
  ...
  try {
    ...
    otherData.value.forEach(item => {
      item.cumulativeIncome = calculateCumulativeIncome('otherData', item.customerType)
    })
...
const loadData = async (targetPeriod: string) => {
  ...
  console.log(`正在加载拓源成本中心数据，期间: ${targetPeriod}`)

  try {
    const response = await fetch(`http://47.111.95.19:3000/tuoyuan-cost-center-structure/${targetPeriod}`)
    if (!response.ok) {
      if (response.status === 404) {
        console.log('该期间暂无数据，使用初始数据')
        const initialData = getInitialData()
        equipmentData.value = initialData.equipmentData
        otherData.value = initialData.otherData
        // 加载所有月份数据并更新累计收入
    ...
    if (result.success && result.data) {
      ...
      otherData.value = (result.data.otherData || getInitialData().otherData).map((item: any) => ({
        ...item,
        marketing: item.marketing || 0,
        management: item.management || 0,
        finance: item.finance || 0,
    ...
    console.log(`拓源成本中心数据加载完成，期间: ${targetPeriod}`)
  } catch (error) {
...
const handleSave = async () => {
  try {
    ...
    // 1. 保存到 tuoyuan_cost_center_structure 表
    const response = await fetch('http://47.111.95.19:3000/tuoyuan-cost-center-structure', {
      method: 'POST',
      headers: {
