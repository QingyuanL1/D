<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
      <!-- 页面标题 -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">南华新签订单数据分析</h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-1 gap-6 mb-8">
        <div class="bg-white rounded-lg shadow-sm p-6">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">工程类别</h3>
          <div class="space-y-3">
            <div class="flex justify-between">
              <span class="text-sm text-gray-600">年度计划:</span>
              <span class="font-medium text-blue-600">{{ formatNumber(summary['工程']?.yearly_plan || 0) }} 万元</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-600">当期累计:</span>
              <span class="font-medium text-green-600">{{ formatNumber(summary['工程']?.current_total || 0) }} 万元</span>
            </div>
            <div class="flex justify-between">
              <span class="text-sm text-gray-600">完成率:</span>
              <span class="font-medium" :class="getCompletionRateColor(summary['工程']?.completion_rate || 0)">
                {{ summary['工程']?.completion_rate || 0 }}%
              </span>
            </div>
...
const totalYearlyPlan = computed(() => {
  return summary.value['工程']?.yearly_plan || 0
})

const totalCumulative = computed(() => {
  return summary.value['工程']?.current_total || 0
})

const completionRate = computed(() => {
  return summary.value['工程']?.completion_rate || 0
})
...
const fetchNewOrdersData = async () => {
  try {
    const response = await fetch(`http://47.111.95.19:3000/analytics/nanhua-new-orders/${selectedYear.value}`)
    if (response.ok) {
...
const fetchPieData = async () => {
  try {
    const response = await fetch(`http://47.111.95.19:3000/analytics/nanhua-new-orders-breakdown/${selectedYear.value}`)
    if (response.ok) {
      const result = await response.json()
      if (result.success && result.data) {
...
const updateTrendChart = () => {
  ...
  if (hasData && monthlyData.value['工程']) {
    ...
    series.push({
      name: '累计新签订单',
      type: 'line',
      data: engineeringData.cumulative || [],
      smooth: true,
      lineStyle: {
        color: '#3B82F6',
        width: 3
    ...
    series.push({
      name: '当月新签订单',
      type: 'bar',
      data: engineeringData.current_total || [],
      itemStyle: {
        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
  ...
  const option = {
    title: {
      text: `${selectedYear.value}年南华新签订单趋势分析`,
      textStyle: {
        fontSize: 18,
        fontWeight: 'bold',
        color: '#374151'
