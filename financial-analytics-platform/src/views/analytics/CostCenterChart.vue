...
const fetchCostCenterData = async () => {
  try {
    const response = await fetch(`http://127.0.0.1:3000/analytics/cost-center/${selectedYear.value}`)
    if (response.ok) {
      ...
      if (categoryData) {
        ...
        if (selectedYear.value !== '2025') {
          series.push({
            ...
            barWidth: '15%', // 减小单个柱子宽度以便分组显示
            barGap: '10%', // 设置柱子间距
            barCategoryGap: '20%' // 设置分类间距
          })
        }
      }
    })
